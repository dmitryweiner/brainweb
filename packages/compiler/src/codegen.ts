...
export function generateJS(ir: AppIR): string {
  const lines: string[] = [];
  ...
  // Debug timing + stats
  emit(`  var _bwStats = { ticks: 0, events: 0, actions: 0, lastFpsTime: 0, fps: 0, eventsPerSec: 0, actionsPerSec: 0, _evAcc: 0, _actAcc: 0 };`);
  emit(`  var _bwPT = [];`);
  emit(``);
  ...
  emit(`      var _t0 = performance.now();`);
  emit(`      var _qLen = eventQueue.getLength();`);
  emit(`      lastEvents = eventQueue.drain();`);
  emit(`      _bwStats._evAcc += lastEvents.length;`);
  emit(`      for (const ev of lastEvents) {`);
  emit(`        recorder.recordEvent(ev);`);
  emit(`      }`);
  emit(`      if (lastEvents.length > 0) debugOverlay.update({ eventQueueLen: _qLen });`);
  emit(`      _bwPT[0] = { name: "ingest", durationMs: performance.now() - _t0 };`);
  emit(`    },`);
  ...
  if (encoder) {
    ...
    emit(`      var _t0 = performance.now();`);
    emit(`      if (lastEvents.length === 0) { _bwPT[1] = { name: "encode", durationMs: 0 }; return; }`);
    ...
    emit(`      _bwPT[1] = { name: "encode", durationMs: performance.now() - _t0 };`);
    emit(`    },`);
  }
  ...
  if (actionSel) {
    ...
    emit(`      var _t0 = performance.now();`);
    emit(`      // Update tick stats`);
    emit(`      _bwStats.ticks++;`);
    emit(`      if (now - _bwStats.lastFpsTime >= 500) {`);
    emit(`        var dt = (now - _bwStats.lastFpsTime) / 1000;`);
    emit(`        if (dt > 0) {`);
    emit(`          _bwStats.fps = _bwStats.ticks / dt;`);
    emit(`          _bwStats.eventsPerSec = _bwStats._evAcc / dt;`);
    emit(`          _bwStats.actionsPerSec = _bwStats._actAcc / dt;`);
    emit(`        }`);
    emit(`        _bwStats.ticks = 0;`);
    emit(`        _bwStats._evAcc = 0;`);
    emit(`        _bwStats._actAcc = 0;`);
    emit(`        _bwStats.lastFpsTime = now;`);
    emit(`      }`);
    emit(`      if (lastEvents.length === 0) {`);
    emit(`        _bwPT[2] = { name: "action", durationMs: 0 };`);
    emit(`        debugOverlay.update({`);
    emit(`          tickStats: { fps: _bwStats.fps, eventsPerSec: _bwStats.eventsPerSec, actionsPerSec: _bwStats.actionsPerSec, tickMs: 0 },`);
    emit(`          pipelineSteps: _bwPT.slice()`);
    emit(`        });`);
    emit(`        return;`);
    emit(`      }`);
    ...
    emit(`      if (allowed) _bwStats._actAcc++;`);
    emit(``);
    emit(`      _bwPT[2] = { name: "action", durationMs: performance.now() - _t0 };`);
    emit(`      var _totalTick = 0;`);
    emit(`      for (var _ti = 0; _ti < _bwPT.length; _ti++) _totalTick += _bwPT[_ti] ? _bwPT[_ti].durationMs : 0;`);
    emit(``);
    emit(`      // Debug update`);
    emit(`      debugOverlay.update({`);
    ...
    if (encoder) {
      emit(`        featureVector: Array.from(encoder.output),`);
      emit(`        encoderOps: encoder.getOpRanges(),`);
    }
    if (ctxModule) {
      emit(`        contextSlots: contextMemory.getSlotInfo(now),`);
    }
    emit(`        guardHistory: guards.getRecentEffects(),`);
    emit(`        weightMatrix: { weights: Array.from(actionSelector.weights), actions: actions, featureDim: actionSelector.featureDim },`);
    emit(`        pipelineSteps: _bwPT.slice(),`);
    emit(`        tickStats: { fps: _bwStats.fps, eventsPerSec: _bwStats.eventsPerSec, actionsPerSec: _bwStats.actionsPerSec, tickMs: _totalTick },`);
    emit(`      });`);
    emit(``);
    emit(`      // Record`);
