// ── BrainWeb Intermediate Representation ──
...
export interface SensorPatternIR {
  sensor: string; // "*" for all
  event: string;  // "*" for all
}

export type FeatureOpIR =
  | { kind: "onehot"; field: string }
  | { kind: "bucket"; field: string; bins: number }
  | { kind: "hash"; field: string; buckets: number }
  | { kind: "numeric"; field: string }
  | { kind: "clamp"; field: string; min: number; max: number }
  | { kind: "scale"; field: string; factor: number };

// ── Modules (populations compiled to runtime modules) ──

export type ModuleIR =
  | StateModuleIR
  | SpikingModuleIR
  | RecurrentModuleIR
  | ActionSelectorModuleIR
  | RateModuleIR;
...
export interface EffectorBindingIR {
  action: string;
  kind: "js" | "noop";
  expr?: string; // the js("...") content for kind=js
}
