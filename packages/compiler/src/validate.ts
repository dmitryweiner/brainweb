...
export interface SymbolTable {
  ...
  allModules: Set<string>; // all runnable module names
}

export function validate(app: AppNode): Diagnostic[] {
...
function buildSymbolTable(app: AppNode, diags: Diagnostic[]): SymbolTable {
...
function registerRegion(region: RegionNode, symbols: SymbolTable, diags: Diagnostic[]): void {
...
function resolveNames(app: AppNode, symbols: SymbolTable, diags: Diagnostic[]): void {
  for (const decl of app.declarations) {
...
function resolveModuleRef(name: string, symbols: SymbolTable, scope?: string): boolean {
  if (symbols.allModules.has(name) || symbols.populations.has(name)) return true;
  // Try qualifying with scope prefix (circuit or region)
  if (scope) {
    const qualified = `${scope}.${name}`;
    if (symbols.allModules.has(qualified) || symbols.populations.has(qualified)) return true;
  }
  return false;
}

function checkProjectionRefs(proj: ProjectionNode, symbols: SymbolTable, diags: Diagnostic[], scope?: string): void {
...
function checkTypeSanity(app: AppNode, diags: Diagnostic[]): void {
  for (const decl of app.declarations) {
    if (decl.kind === "Encoder") {
      if (decl.outDim <= 0) {
...
function checkPopSanity(pop: PopulationNode, diags: Diagnostic[]): void {
...
function checkBindingSafety(app: AppNode, diags: Diagnostic[]): void {
  for (const decl of app.declarations) {
