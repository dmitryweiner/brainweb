...
exports.Recorder = exports.DebugOverlay = void 0;
class DebugOverlay {
    constructor(enabled) {
        this.container = null;
        this.state = {
    ...
    render() {
        if (!this.container)
            return;
        const s = this.state;
        const eventsHtml = s.lastEvents.slice(-5).map(e => `<div style="padding:1px 0;color:#8cf">${e.sensor}.${e.type} <span style="color:#666">${e.payload?.target ?? ""}</span></div>`).join("");
...
class Recorder {
    ...
    recordAction(t, winner, guardRejected) {
        if (!this.recording)
            return;
